...
public class EmpDeptSalgradeTests
{
    ...
    public void ShouldReturnEmployeesFromChicago()
    {
        ...
        List<Emp> result = emps.Where(emp => depts.Where(dept => dept.Loc.Equals("CHICAGO")).Select(dept => dept.DeptNo).Contains(emp.DeptNo)).ToList(); 
    ...
    public void ShouldCountEmployeesPerDepartment()
    {
        ...
        var result = emps.GroupBy(emp => emp.DeptNo).Select(e => new { DeptNo = e.Key, Count = e.Count() }).ToList(); 
    ...
    public void ShouldMatchEmployeeToSalaryGrade()
    {
        ...
        var result = emps.SelectMany(emp =>
            grades.Where(grade => grade.Losal <= emp.Sal && grade.Hisal >= emp.Sal).Select(grade => new {emp.EName, grade.Grade})).ToList();
    ...
    public void ShouldCalculateAverageSalaryPerDept()
    {
        ...
        var result = emps.GroupBy(emp => emp.DeptNo).Select(e => new {DeptNo = e.Key, AvgSal = e.Average(em => em.Sal)}).ToList(); 
    ...
    public void ShouldReturnEmployeesEarningMoreThanDeptAverage()
    {
        ...
        var result = emps.Where(emp => emp.Sal > emps.Where(e => e.DeptNo == emp.DeptNo).Average(em => em.Sal)).Select(emp => emp.EName).ToList(); 
